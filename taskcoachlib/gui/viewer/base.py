...
class Viewer(wx.Panel, patterns.Observer, metaclass=ViewerMeta):
    ...
    def initLayout(self):
        ...
        # Use SetSizer instead of SetSizerAndFit to prevent locking MinSize
    ...
    def GetEffectiveMinSize(self):
        ...
        Since child widget's BestSize can be 3700px+ (sum of all columns),
        this prevents the Viewer from shrinking below that even with proportion=1 and wx.EXPAND.
        ...
        print(f"[Viewer] GetEffectiveMinSize() called - MinSize: {minSize}, BestSize: {self.GetBestSize()}, Returning: {effectiveMin}, Current Size: {self.GetSize()}")
        return effectiveMin
